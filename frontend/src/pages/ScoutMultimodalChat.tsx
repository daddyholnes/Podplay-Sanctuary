...
import { 
  ...
  Sun, Moon, Copy, ThumbsUp, ThumbsDown, RotateCcw, X,
  Zap, Eye, Clock, DollarSign, Database, Sparkles, Brain
} from 'lucide-react';
import julesLogo from '../assets/logos/jules.png';
import geminiLogo from '../assets/logos/gemini.png';

export const ScoutMultimodalChat = () => {
  ...
  const models = {
    ...
    'gemini-2.5-pro-preview-05-06': {
      ...
      provider: 'Google',
      icon: <img src={geminiLogo} alt="Gemini" className="w-full h-full object-contain" />,
    ...
    'jules': {
      name: 'Jules',
      provider: 'Google',
      icon: <img src={julesLogo} alt="Jules" className="w-full h-full object-contain" />,
  ...
  const CapabilityBadge = ({ capability }: { capability: string }) => {
    ...
    const config = capabilityIcons[capability as keyof typeof capabilityIcons] || capabilityIcons.code;
      ...
      <div className="flex-1 flex flex-col">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {('badge' in currentModel) && currentModel.badge && (
            ...
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg shadow-lg ${
                message.type === 'user' 
                  ...
                  : `bg-gradient-to-r ${message.model ? models[message.model as keyof typeof models]?.color : 'from-gray-500 to-gray-600'}`
              }`}>
                {message.type === 'user' ? '👤' : (message.model ? models[message.model as keyof typeof models]?.icon : '🤖')}
              </div>
        ...
        <div className={`${themeClasses.cardBg} backdrop-blur-md border-t ${themeClasses.border} p-4`}>
          ...
          <input
            ...
            hidden
            multiple
            onChange={(e: ChangeEvent<HTMLInputElement>) => handleFileUpload(e.target.files, 'file')}
          />
          <input
            ref={imageInputRef}
            type="file"
            ...
            multiple
            accept="image/*"
            onChange={(e: ChangeEvent<HTMLInputElement>) => handleFileUpload(e.target.files, 'image')}
          />
          <input
            ref={videoInputRef}
            type="file"
            ...
            multiple
            accept="video/*"
            onChange={(e: ChangeEvent<HTMLInputElement>) => handleFileUpload(e.target.files, 'video')}
          />
        </div>
      </div>
    </div>
